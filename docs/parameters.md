...
| `arm_named_pose_action_name` | `move_arm_to_named_pose` | Name of the `grasping_msgs/action/MoveToNamedPose` action server used when `do_post_grasp_move` requests the configured `post_grasp` pose. |
...
| `do_post_grasp_move` | `true` | Requests the control stack's configured `post_grasp` named pose after a successful close when enabled. |
